...
public class DictionaryScreen extends BorderPane {
    ...
    public DictionaryScreen(String username) {
        ...
        favoritesButton.setWrapText(true); // Enable text wrapping
    ...
    private void performSearch(String query, Button searchButton) {
        ...
        if (definition != null && !definition.isEmpty()) {
            ...
            definitionLabel.setWrapText(true); // Enable text wrapping
            definitionLabel.setMaxWidth(500);
...
//        new Thread(() -> {
    ...
    // Update the list of favorite words for the current user
    private void updateFavoritesList() {
        // Clear the current items in the favorites list
        favoriteWords.getItems().clear();

        try {
            // Prepare an SQL statement to retrieve the user's favorite words
            PreparedStatement stmt = connection.prepareStatement(
            ...
            int count = 1; // Initialize a counter for numbering the favorite words
            while (rs.next()) {
                // Retrieve the word, its context, and the date it was added from the result set
                String word = rs.getString("word");
                String context = rs.getString("context");
                String date = rs.getString("date");

                // Format the data into a readable format for display
                String formattedItem = String.format(
                        "%d. %s (Added: %s)\n   Context:\n   %s",
                        count, word, date, context.replace("\n", "\n   ")
                );

                // Add the formatted item to the favorites list view
                favoriteWords.getItems().add(formattedItem);

                // Increment the counter for the next favorite word
                count++;
            }
        } catch (SQLException e) {
            // Print the stack trace if there is an error with the database query
            e.printStackTrace();
        }
    }

    // Toggle between showing the search results and the user's favorites list
    private void toggleFavoritesView() {
        // Check which view is currently being displayed
        if (getCenter() == searchResults) {
            // If the search results are displayed, switch to the favorites list
            updateFavoritesList(); // Refresh the favorites list
            setCenter(favoriteWords); // Set the center to display the favorites
        } else {
            // If the favorites list is displayed, switch back to the search results
            setCenter(searchResults); // Set the center to display the search results
        }
    }
}